...
/* Stayin' Alive:                                                                                    */
/*                                                                                                   */
/* This script will try to keep you alive by any means possible, either by using healer skills or    */
/* buying health potions. If you are not a healer or you don't have enough mana to cast healer       */
/* skills it will try to buy a health potion for you.                                                */
/*                                                                                                   */
/*-------------------------------------------------------------------------------------------------- */
...
var healthTrigger = 50;
// Used to monitor your health: if it falls below this number, the script is activated. Change it to your liking.

var tryBlessingFirst = false;
// If you are a healer and would like to try "Blessing" first instead of "Healing Light", change this to true.
// P.S.: You must be at least level 14 to use this skill.

/*-------------------------------------------------------------------------------------------------- */
/*             Do not edit code below this line (or do edit if you to try your own ideas)            */
/*-------------------------------------------------------------------------------------------------- */
...
function StayingAlive() {
 var response = JSON.parse(
  UrlFetchApp.fetch('https://habitica.com/api/v3/user', {
   method: 'get',
   headers,
  })
 );
 ...
 if (myStats.hp < healthTrigger) {
  ...
  if (
   myStats.class == 'healer' &&
   myStats.mp >= healingLightManaCost
  ) {
   ...
   if (tryBlessingFirst && myStats.mp >= blessingManaCost) {
    // Casts "Blessing"
    UrlFetchApp.fetch(
     'https://habitica.com/api/v3/user/class/cast/healAll',
     {method: 'post', headers}
    );
   }
   // User has chosen to cast "Healing Light" OR they don't have enough mana for "Blessing"
    ...
    // Casts "Healing Light"
    UrlFetchApp.fetch(
     'https://habitica.com/api/v3/user/class/cast/heal',
     {method: 'post', headers}
    );
   }
  }
  // User is not a healer OR do not have enough mana to use a healer skill
  else if (myStats.gp >= healthPotionGoldCost) {
   // Checks if user has enough gold to buy a health potion
